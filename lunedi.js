...
function addExercise(exercise = { name: "", reps: "", sets: "", weight: "", sec:"", image: "" }) {
    const exerciseList = document.getElementById("exercise-list");
    const exerciseDiv = document.createElement("div");
    exerciseDiv.className = "exercise";
    exerciseDiv.innerHTML = `
        <input type="text" placeholder="Nome esercizio" value="${exercise.name}">
        <input type="number" placeholder="Serie" value="${exercise.sets}">
        <input type="number" placeholder="Ripetizioni" value="${exercise.reps}">
        <input type="number" placeholder="Peso (kg)" value="${exercise.weight}">
        <input type="number" placeholder="Riposo (s)" value="${exercise.sec}">
        <input type="file" accept="image/*" onchange="previewImage(event, this)" data-image="${exercise.image}">
        <img src="${exercise.image}" alt="Immagine Esercizio" ${exercise.image ? '' : 'style="display:none;"'}>
...
function saveExercises() {
    ...
    const exercises = [];
    document.querySelectorAll("#exercise-list .exercise").forEach(exerciseDiv => {
    ...
    alert("Esercizi salvati per Lunedì!");
}
...
function loadExercises() {
    ...
    const exercises = JSON.parse(localStorage.getItem(`exercises_${userName}`) || "[]");
